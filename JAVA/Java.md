...
2. 在子类的构造方法中使用`super()`调用父类构造方法时，`super()`必须出现在构造方法的第一行。而`this()`也需要在第一行(该方法用于调用类中其他已定义的构造方法)。`super()`与`this()`无法同时使用。
### 11. JNDI
### 12. 异常
...
注意：由于Java 字符串中反斜杠本身需要转义，因此两个反斜杠（`\\`）实际上相当于一个（前一个用于转义）。
#### 预定义字符
可以匹配多个字符的特殊字符：
* .	可以匹配任何字符
* \d	匹配 0~9 的所有数字
