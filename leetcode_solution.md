...
##### 技巧
1. 去重：
   * 对于可排序的输入，先对输入排序，在使用大小为input.length的辅助空间，如`boolean[] is_used = new boolean[input.length]`，当`i>0&&input[i]==input[i-1]&&!is_used[i-1]`时，表示出现同层的重复。
   * 对于不可排序的输入，可在每层新建map、set或数组(当输入范围较小时),判断是否出现同层重复。
#### **递归算法**
递归三要素
...
```
3. union()合并集合,即使一个根节点的前驱节点指向另一个根节点。
```java
...
当前问题的解可由其子问题的解推导出来。
##### 01背包
即若干个物品每个代价为n，收益为m，每个物品只能取一次，求消耗一定代价的情况下，取得的最高收益。
常用dp代码：
```java
for(int i = 0; i < weight.size(); i++) { // 遍历物品，物品在外层保证每个背包都有放进多个物品的可能
    for(int j = bagWeight; j >= weight[i]; j--) { // 遍历背包容量，倒序保证每个物品只放入背包一次
...
```
##### 完全背包
即若干个物品每个代价为n，收益为m，每个物品只能取无限次，求消耗一定代价的情况下，取得的最高收益。
常用dp代码：
```java
for(int i = 0; i < weight.size(); i++) { // 遍历物品，内外层可颠倒
...
**注：**：
1. 先物品后容量为组合，
2. 先容量后物品为排列,且可用于字符串匹配
##### 多重背包
即若干类物品每类代价为n，收益为m，每类物品有ki个（i对应类数），每个物品只能取一次，求消耗一定代价的情况下，取得的最高收益。
可以当做时特殊的01背包处理，即有重复代价的物品。
#### 技巧与注意事项
